...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 清风")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'SM3BZ-4S5LJ-UDCF5-FI7PA-UQTBE-LJBOA',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(121.45442,37.47004, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 清风") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('清风的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://limh.cc/assets/ikun.png' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://img.shields.io/badge/%E6%B8%85%E9%A3%8E-%E4%B8%8B%E7%8F%AD%E5%95%A6%E6%AC%A2%E8%BF%8E%E4%B8%8B%E6%AC%A1%E5%85%89%E4%B8%B4-orange' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(/img/bg.webp);
    --darkmode-bg: url(/img/bgyejian.webp);
    --mobileday-bg: url(/img/shoujibg.webp);
    --mobilenight-bg: url(/img/shoujibgye.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
